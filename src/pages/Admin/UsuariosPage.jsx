...
export default function UsuariosPage() {
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      setError(
        'No se pudo conectar con el servidor para obtener la lista de usuarios.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleToggleRole = async (userId, currentRole) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || 'Error al intentar actualizar el rol.'
      );
    }
  };
  ...
  const handleSuspendUser = async (userId) => {
    if (
      !window.confirm(
        '¿Está seguro de que desea suspender esta cuenta de usuario? El usuario no podrá iniciar sesión.'
      )
    ) {
      return;
    }
    try {
      ...
      if (response.success) {
        triggerSuccessFeedback(
          'Usuario suspendido (baja lógica) correctamente.'
        );
        loadUsers();
      }
    } catch (err) {
      ...
      setError(
        err.response?.data?.message ||
          'No se pudo suspender la cuenta del usuario.'
      );
    }
  };
  ...
  const handleRestoreUser = async (userId) => {
    try {
      ...
      setError(
        err.response?.data?.message ||
          'No se pudo reactivar la cuenta del usuario.'
      );
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
            Administra los roles, suspensiones lógicas (LGPDPPSO) y cuentas de
      ...
      {error && (
        <div className="p-4 rounded-xl bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 text-xs text-red-700 dark:text-red-400 font-semibold flex items-center justify-between gap-2 animate-in slide-in-from-top-2">
          ...
          <button
            ...
            className="font-bold uppercase text-[10px] tracking-wider text-red-500 hover:text-red-700 cursor-pointer"
          >
            Descartar
          </button>
        </div>
        ...
        {loading ? (
          <div className="p-12 flex flex-col items-center justify-center gap-3">
            ...
            <span className="text-xs text-zinc-400 font-semibold">
              Consultando base de datos MongoDB...
            </span>
          </div>
        ) : users.length === 0 ? (
          <div className="p-12 text-center space-y-2">
            ...
            <p className="text-xs text-zinc-500">
              No existen cuentas registradas en la base de datos de MongoDB.
            </p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/60">
                {filteredUsers.map((user) => {
                  const displayRole =
                    user.role === 'admin' ? 'Fotógrafo / Admin' : 'Cliente';
                  const isDeleted =
                    user.deletedAt !== null || user.status === 'inactive';
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-zinc-50/50 dark:hover:bg-zinc-850/20 transition-colors ${
                        isDeleted
                          ? 'opacity-60 bg-zinc-50/20 dark:bg-zinc-950/10'
                          : ''
                      }`}
                    >
                      {/* Name & Avatar */}
                      ...
                      <td className="px-6 py-4.5">
                        ...
                        >
                          <span
                            className={`w-1 h-1 rounded-full ${isDeleted ? 'bg-red-500' : 'bg-emerald-500'}`}
                          />
